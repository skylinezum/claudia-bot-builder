...
const asyncLambdaRecursion = require('../async-lambda-recursion');
const color = require('../console-colors');

module.exports = function slackSetup(api, bot, logError, optionalParser, optionalResponder, optionalAsyncInvoker, optionalAsyncResponder) {
  let parser = optionalParser || slackParse,
    responder = optionalResponder || slackReply,
    asyncInvoker = optionalAsyncInvoker || asyncLambdaRecursion,
    asyncResponder = optionalAsyncResponder;
  const syncSlashProcessing = (request) => {
  ...
  const asyncSlashProcessing = (request) => {
    return bot(parser(request.post), request)
      .then(asyncResponder)
      .catch(logError);
  };
  api.post('/slack/slash-command', request => {
    if (request.env.slackResponse === 'delayed') {
      if (request.async) {
        return asyncSlashProcessing(request);
      } else {
        request.async = true;
        return asyncInvoker(request);
      }
    } else {
      return syncSlashProcessing(request);
    }
  });
